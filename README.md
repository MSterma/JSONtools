...
>  GUI znajduje się pod /swagger-ui/index.html#/

---
## Diagram UML najważniejszych klas
![diagram_uml](JSONTools_UML.png)
## Product Backlog
...
| Jako użytkownik mogę skorzystać z aplikacji za pomocą interfejsu użytkownika | | | | o Użytkownik może wprowadzić lub wkleić dane do pól tekstowych<br>o Użytkownik może wybrać dostępną funkcję (np. porównanie tekstów, sortowanie pól JSON, konwersja JSON do CSV)<br>o Wyniki operacji są prezentowane w przejrzysty sposób<br>o Aplikacja poprawnie obsługuje błędy i wyświetla komunikaty dla użytkownika |
| Jako programista mogę porównać wiele tekstów (np.<br>reprezentujące strukturę danych w formatach JSON) oczekując, że aplikacja pokaże linijki, w których występuje różnica. | | | | o Prawidłowo podawane linie z różnicami<br>o Dla kilku pustych tekstów nie są znajdowane różnice między nimi<br>o Dla jednego pustego tekstu (w przypadku gdy inne nie są puste) cały tekst zaznaczany jest jako różnica |
| Jako programista moge uzyskać strukturę w formacie JSON zawierająca pola posortowane alfabetyznie. | | | |  o Kształt danych nie ulega zmianie<br> o W zapisie pola zmieniają swoją kolejność na alfabetyczną  <br>o Zgłoszenie błędu dla niepoprawnego JSON-a |
| Jako programista mogę skonwertować dane zapisane w formacie JSON do formatu CSV, aby móc je dalej analizować w narzędziach obsługujących CSV. | | | | o Poprawna konwersja prawidłowego JSON-a do CSV<br>o Nagłówki CSV tworzone na podstawie nazw pól w JSON-ie<br>o Zachowanie wszystkich wartości danych po konwersji<br>o Zgłoszenie błędu dla niepoprawnego lub nieobsługiwanego formatu JSON |
